...
public class GearClaw {
	...
	public boolean canOpenPanel = true; //used to determine if we can legally open the pickup panel
	...
	public GearClaw(int PCMCanID, int PCMForwardChannel, int PCMReverseChannel, int CANTalonID, double TalonP, double TalonI, double TalonD){
		...
		pivotMotor.configMaxOutputVoltage(5); //goes to 12 or -12 for max speed
	...
	public void stateMachine(){
			...
			case PICKUP:
				...
				break;
			case CLAMP:
				canOpenPanel = false; //if we are in clamp mode we cannot deploy the pickup panel
				down();
				retract();
				break;
			case LIFT: 
				up();
				retract();
				canOpenPanel = true; //if we are in lift mode the panel will be able to deploy legally
				break;
			case READYTOSCORE:
				score();
				retract();
	...
	public boolean getOpenPanelStaus() { return canOpenPanel; }
}
